...
function displayData(arrivals, departures) {
	...
	// 念のためここでも降順を保証
	const sortedArrivals = [...arrivals].sort((a, b) => b.timestamp.localeCompare(a.timestamp))
	...
	sortedArrivals.forEach(item => {
		...
		editBtn.addEventListener("click", () => {
			const [dateOnly] = item.timestamp.split("T")
			window.location.href = `/adminedit?date=${dateOnly}&name=${encodeURIComponent(item.userName)}`
		})
		...
		deleteBtn.addEventListener("click", () => {
			if (confirm(`${item.userName} の記録を削除しますか？`)) {
				...
				const departure = departures.find(dep =>
					dep.userName === item.userName && dep.timestamp.startsWith(dateOnly)
				)
				if (departure) {
